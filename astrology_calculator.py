...
# --- Custom element & quality calculation (restricted to planets + Asc + MC) ---
_SIGN_TO_ELEMENT = {
...
def _build_elements_and_qualities(subject) -> str:
    """Calculate elemental and quality balance using ONLY the 10 planets + Ascendant + Midheaven."""
    selected = [
    ...
    for point in selected:
        ...
        if sign:
            sign = sign[:3]
            if elem := _SIGN_TO_ELEMENT.get(sign):
                elements[elem] += 1
            if qual := _SIGN_TO_QUALITY.get(sign):
                qualities[qual] += 1
    ...
    for elem in ('Fire', 'Earth', 'Air', 'Water'):
        c = elements[elem]
        out += f"{elem:6} : {c:2d} / {total}   ({(c/total)*100:5.1f}%)\n"

    out += "\nQUALITY / MODALITY BALANCE (same restricted points)\n" + "=" * 55 + "\n"
    for q in ('Cardinal', 'Fixed', 'Mutable'):
        c = qualities[q]
        out += f"{q:8} : {c:2d} / {total}   ({(c/total)*100:5.1f}%)\n"

    out += "\n(Note: Custom calculation using only the requested points.)\n"
    return out
...
def generate_practical_report():
    ...
    elem_qual = _build_elements_and_qualities(subject)
    ...
    txt_content += report_str + quick + aspects + elem_qual
...
<h2>Elements &amp; Qualities (10 planets + Asc + MC only)</h2><pre>{elem_qual}</pre>
